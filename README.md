Some additional works and tips on Edward Choi's medGAN
====
...
* `04_medgan-for-data-agumentation.ipynb`: [Using `medGAN` to try to perform data augmentation on the MIMIC-III dataset with binary values](https://github.com/sylvaincom/medgan-tips/blob/master/04_medgan-for-data-augmentation.ipynb). In this notebook, I try to use `medGAN` to perform data augmentation, thus to boost prediction performance.
